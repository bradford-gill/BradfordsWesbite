...
export default function RedsoxGreenJerseys() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch('/blog/redsox-green-jerseys/baseline_stats.json').then((r) => r.json()),
      fetch('/blog/redsox-green-jerseys/metadata.json').then((r) => r.json()),
    ]).then(([csv, baseline, meta]) => {
      const lines = csv.trim().split('\n').slice(1);
      setGames(
        lines.map((line) => {
          const [date, opponent, result, sox_score, opp_score, innings, walkoff] = line.split(',');
          return {
            date,
            opponent,
  ...
  return (
    <div style={{ fontFamily: 'inherit' }}>
        ...
        <div style={{ marginBottom: '1.75rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          ...
          <span style={{ fontSize: '0.72rem', color: '#888' }}>
            ...
            <strong style={{ color: '#555' }}>
              {new Date(lastRefreshed + 'T12:00:00').toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}
            </strong>
          </span>
        </div>
      )}
      ...
      >
        {[
          { label: 'Record', value: `${wins}–${losses}`, sub: `${(winPct * 100).toFixed(1)}% win rate` },
          { label: 'Walk-offs', value: `${walkoffs}`, sub: `of ${total} games` },
          { label: 'Walk-offs / Win', value: `${(walkoffOfWins * 100).toFixed(1)}%`, sub: `${walkoffs} of ${wins} wins` },
          ...
          >
            <div style={{ fontSize: '0.68rem', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#7fba9a', marginBottom: '0.4rem' }}>
              {label}
            </div>
            <div style={{ fontSize: '2rem', fontWeight: 900, color: '#fff', lineHeight: 1 }}>{value}</div>
            <div style={{ fontSize: '0.72rem', color: '#9fcfaf', marginTop: '0.3rem' }}>{sub}</div>
          </div>
        ))}
      </div>
        ...
        <div style={{ marginBottom: '2.5rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '0.75rem', marginBottom: '1rem' }}>
            {[
              ...
              >
                <div style={{ fontSize: '0.68rem', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: GREEN_DARK, marginBottom: '0.35rem' }}>
                  {label}
                </div>
                <div style={{ fontSize: '1.75rem', fontWeight: 900, color: GREEN_DARK }}>{value}</div>
                <div style={{ fontSize: '0.72rem', color: '#4a7a5c', marginTop: '0.25rem', lineHeight: 1.4 }}>{sub}</div>
              </div>
            ))}
          </div>
          <p style={{ fontSize: '0.78rem', color: '#555', lineHeight: 1.6, background: GREEN_LIGHT, border: `1px solid ${GREEN}`, padding: '0.75rem 1rem' }}>
            Binomial test: if each green jersey win had the same {(stats.baseline_walkoff_rate * 100).toFixed(1)}% walk-off probability as any MLB home win from 2000–2024,
            the probability of seeing {stats.sox_walkoffs} or more walk-offs in {stats.sox_wins} wins is <strong>{(stats.prob_at_least_sox_walkoffs * 100).toFixed(2)}%</strong>.
            The observed rate is <strong>{((stats.sox_walkoff_rate / stats.baseline_walkoff_rate)).toFixed(1)}×</strong> the historical average.
          </p>
      ...
      <div>
            ...
            <tbody>
              {games.map((game, i) => (
                <tr
                  key={game.date}
                  style={{
                    background: game.walkoff ? '#d4edda' : i % 2 === 0 ? '#fff' : CREAM,
                    borderBottom: '1px solid #ddd',
                  }}
                >
                  <td style={{ padding: '0.6rem 1rem', color: '#555', whiteSpace: 'nowrap' }}>{game.date}</td>
                  <td style={{ padding: '0.6rem 1rem', fontWeight: 500 }}>{game.opponent}</td>
                  <td style={{ padding: '0.6rem 1rem', fontWeight: 700 }}>
                    {game.sox_score}–{game.opp_score}
                    {game.innings > 9 ? <span style={{ fontWeight: 400, fontSize: '0.75rem', color: '#888', marginLeft: '0.3rem' }}>({game.innings})</span> : null}
                  </td>
        ...
        <p style={{ fontSize: '0.72rem', color: '#888', marginTop: '0.5rem' }}>
          Data via MLB Stats API. Green jersey Fridays = all Red Sox regular-season home games on Fridays, 2025–2026.
        </p>
      </div>
...
function Narrative({ stats }: { stats: BaselineStats }) {
  ...
  return (
    ...
    >
      <p style={prose}>
        On April 17th, 2026, the Red Sox put on their Friday greens at Fenway and, once again, walked it off. It's
        become almost expected at this point. The green jerseys come out, the crowd stays late, and somehow the Sox
        find a way to win it in their last at-bat. There's something about those jerseys.
      </p>
      <p style={prose}>
        But how improbable is it, really? That question sent me to the MLB Stats API. I pulled every Red Sox
        regular-season home game played on a Friday since 2025 (the green jersey era) and checked each one for a
        walk-off using the game's linescore: if the home team was still batting when the final out was recorded, it
        counts. Sixteen games in, the Sox are {stats.sox_walkoffs} for {stats.sox_wins} on walk-off wins in games
        they won. Over half their victories in green have ended in walk-offs.
      </p>
      <p style={prose}>
        To put that in context, I ran the same walk-off detection across every MLB home win from 2000 through 2024,
        {' '}{stats.total_home_wins.toLocaleString()} games. League-wide, about{' '}
        {(stats.baseline_walkoff_rate * 100).toFixed(1)}% of home wins end in a walk-off. At that rate, you'd expect
        roughly {stats.expected_walkoffs} walk-off wins in {stats.sox_wins} chances. The Sox have {stats.sox_walkoffs}.
        The gap between expected and observed isn't close.
      </p>
      <p style={{ ...prose, marginBottom: 0 }}>
        A simple binomial test asks: if each green jersey win had only that {(stats.baseline_walkoff_rate * 100).toFixed(1)}%
        baseline chance of ending in a walk-off, what's the probability of seeing {stats.sox_walkoffs} or more in{' '}
        {stats.sox_wins} tries? The answer is <strong>{(stats.prob_at_least_sox_walkoffs * 100).toFixed(2)}%</strong>.
        The observed walk-off rate is {(stats.sox_walkoff_rate / stats.baseline_walkoff_rate).toFixed(1)}x the historical
        average. Small sample, only {stats.sox_wins} wins, but improbable by any reasonable measure. The jerseys
        might just be magic.
      </p>
    </div>
  );
...
function Assumptions() {
  ...
  return (
    <div style={{ marginTop: '2.5rem', borderTop: `2px solid ${GREEN_LIGHT}`, paddingTop: '1.5rem' }}>
      <button
        onClick={toggle}
        style={{
          display: 'flex',
...
function AssumptionRow({ label, body }: { label: string; body: string }) {
  ...
  return (
    <div style={{ border: `1px solid ${GREEN_LIGHT}`, background: '#fff' }}>
      ...
      >
        <span style={{ fontWeight: 600, fontSize: '0.85rem', color: GREEN_DARK }}>{label}</span>
        <span style={{ color: GREEN, fontWeight: 700, flexShrink: 0 }}>{open ? '−' : '+'}</span>
      </button>
      {open && (
        <div
          style={{
