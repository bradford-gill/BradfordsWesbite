...
export default function WBCSalaryAnalysis() {
  ...
  const getTopEarners = (limit: number = 15) => {
    return [...players].sort((a, b) => b.aav - a.aav).slice(0, limit);
  };
  ...
  const getContractStatusBreakdown = (team: string) => {
    ...
    teamPlayers.forEach((p) => {
      statusCounts[p.contract_status] =
        (statusCounts[p.contract_status] || 0) + 1;
    });
  ...
  return (
    <div className="space-y-8">
        ...
        <div className="overflow-x-auto bg-white rounded-none border-4 border-[#1B1B1B] shadow-[8px_8px_0px_0px_rgba(27,27,27,1)]">
          <table className="w-full">
            <thead className="bg-[#F5F0EC] border-b-4 border-[#1B1B1B]">
              <tr>
                <th className="px-6 py-4 text-left text-xs font-bold text-[#1B1B1B] uppercase tracking-wider">
                  Team
                </th>
                <th className="px-6 py-4 text-right text-xs font-bold text-[#1B1B1B] uppercase tracking-wider">
                  Total AAV
                </th>
                <th className="px-6 py-4 text-right text-xs font-bold text-[#1B1B1B] uppercase tracking-wider">
                  Players
            ...
            <tbody className="divide-y-2 divide-[#1B1B1B]">
              {teamSummaries.map((summary) => (
                ...
                >
                  <td className="px-6 py-4 font-bold text-[#2E2E2E]">
                    {summary.team}
                  </td>
                  <td className="px-6 py-4 text-right font-bold text-[#FF5722]">
                    {formatCurrency(summary.totalAAV)}
                  </td>
                  <td className="px-6 py-4 text-right text-[#2E2E2E]">
                    {summary.players}
                  </td>
                  <td className="px-6 py-4 text-right text-[#2E2E2E]">
                    {summary.mlbPlayers}
                  ...
                  <td className="px-6 py-4 text-right text-[#2E2E2E]">
                    {formatCurrency(summary.avgAAV)}
                  </td>
                  <td className="px-6 py-4">
                    <span
                      className={`px-3 py-1 rounded-none text-white text-xs font-bold border-2 border-[#1B1B1B] ${getTeamColor(summary.team)}`}
                    >
                      {summary.wbcResult}
                    </span>
                  </td>
      ...
      <section className="space-y-12">
        ...
        {/* Top Earners */}
        <div>
          <h4 className="font-bold text-[#1B1B1B] text-xl mb-4">
            Top 15 Highest-Paid Players
          </h4>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      <div className="text-2xl font-bold text-[#FF5722]">
                        {formatCurrency(player.aav)}
                      </div>
                      <div
                        className={`inline-block px-3 py-1 rounded-none text-white text-xs font-bold mt-1 border-2 border-[#1B1B1B] ${getTeamColor(player.national_team)}`}
                      >
          ...
          <div className="space-y-6">
            {teamSummaries.map((summary) => {
              ...
              return (
                <div
                  key={summary.team}
                  className="mb-6 bg-white border-4 border-[#1B1B1B] p-6 rounded-none shadow-[6px_6px_0px_0px_rgba(27,27,27,1)]"
                >
                  <div className="flex items-center justify-between mb-4">
                    <h5 className="font-bold text-[#1B1B1B] text-lg">
                      {summary.team}
                    </h5>
                    <span className="text-sm font-bold text-[#2E2E2E] bg-[#F5F0EC] px-3 py-1 border-2 border-[#1B1B1B]">
                      {total} players
                    </span>
                  </div>
                  <div className="space-y-3">
                    {Object.entries(breakdown).map(([status, count]) => {
                      ...
                      return (
                        <div key={status} className="flex items-center gap-3">
                          <div className="w-40 text-sm font-bold text-[#2E2E2E]">
                            {status}
                          </div>
        ...
        <div>
          <h4 className="font-bold text-[#1B1B1B] text-xl mb-4">
            Pre-Arbitration Players ($770K)
          </h4>
          <div className="space-y-6">
            {teamSummaries.map((summary) => {
              const preArbPlayers = players.filter(
                (p) =>
                  p.national_team === summary.team &&
                  p.contract_status === "Pre-Arb",
              );
              ...
              return (
                <div
                  key={summary.team}
                  className="mb-6 bg-[#F5F0EC] rounded-none p-6 border-4 border-[#1B1B1B] shadow-[6px_6px_0px_0px_rgba(27,27,27,1)]"
                >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {preArbPlayers.map((player, idx) => (
                      <div
                        key={idx}
                        className="bg-white rounded-none p-3 border-2 border-[#1B1B1B] hover:shadow-[3px_3px_0px_0px_rgba(27,27,27,1)] transition-all hover:translate-x-[-1px] hover:translate-y-[-1px]"
                      >
                        <div className="font-bold text-[#1B1B1B]">
                          {player.full_name}
                        </div>
        ...
        <div>
          <h4 className="font-bold text-[#1B1B1B] text-xl mb-4">
            Italy's Roster ($47.9M Total AAV)
          </h4>
          <div className="bg-white rounded-none border-4 border-[#1B1B1B] p-6 shadow-[8px_8px_0px_0px_rgba(27,27,27,1)]">
            <h5 className="font-bold text-[#1B1B1B] mb-4 text-lg border-b-2 border-[#FF5722] pb-2 inline-block">
              Top 10 Players by AAV
            </h5>
            <div className="space-y-2">
              {players
                ...
                .map((player, idx) => (
                  <div
                    key={idx}
                    className="flex items-center justify-between py-3 border-b-2 border-[#F5F0EC] last:border-0 hover:bg-[#F5F0EC] transition-colors px-2"
                  >
                    <div>
